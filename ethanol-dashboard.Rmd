...
- We are born in an era which is the most developed in human history. We have the amount of comfort and convenience which none other generation in the history of mankind ever experienced. Here, Unlike Excel or any other point and click program, consistency and accuracy of report is guaranteed no matter the size. Only within a fraction of time it takes on Excel, a very complicated statistical analysis can be generated.
...
- All prices and value of ethanol are in _US Dollar on per metric ton basis indicated as Value in Million USD._ All quantities are in _Metric tons indicated as (M.Tons)._
...
### **Regionwise Summary**
```{r,message=FALSE}
Regionwise_Summary <- Main %>% group_by(REGION) %>% summarise(`Qty in M.Tons`= format(round(sum(`QTY. (MT)`),2),big.mark=",",digits=2,nsmall=2,scientific=FALSE),`Value in Mil.USD` = format(round(sum(`VALUE ($USD)`)/1000000,2),big.mark=",",digits=2,nsmall=2,scientific=FALSE)) %>% arrange(`Qty in M.Tons`)
...
Main_Qtywise <- Main_Qtywise %>% arrange(desc(Year),Month)
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Productwise_Summary <- Main %>% group_by(Year,Product) %>% summarise(`Qty in M.Tons (Thousands)`= format(round(sum(`QTY. (MT)`)/1000,2),big.mark=",",digits=2,nsmall=2,scientific=FALSE)) %>% arrange(desc(Year))
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Regionwise_Summary <- Main %>% group_by(Year,REGION)%>% summarise(`Qty in M.Tons (Thousands)`=format(round(sum(`QTY. (MT)`)/1000,2),big.mark=",",digits=2,nsmall=2,scientific=FALSE)) %>% arrange(desc(Year))
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Storagewise_Summary <- Main %>% group_by(Year,`Storage Type`)%>% summarise(`Qty in M.Tons (Thousands)`=format(round(sum(`QTY. (MT)`)/1000,2),big.mark=",",digits=2,nsmall=2,scientific=FALSE)) %>% arrange(desc(Year))
...
**I. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Yearwise_Summary<- Main %>% group_by(Year) %>% summarize(`Value in Mil.USD`= round((sum(`VALUE ($USD)`))/1000000,2)) %>% arrange(desc(Year))
...
Main_Valuewise <- Main_Valuewise %>% arrange(desc(Year),Month)
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Productwise_Summary <- Main %>% group_by(Year,Product) %>% summarise(`Value in Mil.USD`= round(sum(`VALUE ($USD)`/1000000),2)) %>% arrange(desc(Year),desc(`Value in Mil.USD`))
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Regionwise_Summary <- Main %>% group_by(Year,REGION)%>% summarise(`Value in Mil.USD`=round(sum(`VALUE ($USD)`/1000000),2)) %>% arrange(desc(Year),desc(`Value in Mil.USD`))
...
**a. Year wise Data**
```{r, message=FALSE,fig.width=12,fig.height=8}
Storagewise_Summary <- Main %>% group_by(Year,`Storage Type`)%>% summarise(`Value in Mil.USD`=round(sum(`VALUE ($USD)`/1000000),2)) %>% arrange(desc(Year))
...
Main_QtyValuewise <- Main_QtyValuewise %>% arrange(desc(Year),Month)
...
* `By Quantity following has major contribution`
```{r,message=FALSE}
Country_Investigate <- Main %>%  filter(Year==c("2019","2020","2021","2022"))
Country_Investigate<- Country_Investigate %>% group_by(COUNTRY) %>% summarise(Qty=sum(`QTY. (MT)`)) %>% arrange(desc(Qty))
kable(Country_Investigate[1,1])
```

**a. Year Wise Summary**
```{r,message=FALSE,warning=FALSE}
Countrywise_Summary <- Main %>% filter(Year==c("2019","2020","2021","2022"))
Countrywise_Summary <- Countrywise_Summary %>% group_by(Year,COUNTRY) %>% summarise(`Qty in M.Ton (Thousands)`= round(sum(`QTY. (MT)`)/1000,2)) %>% arrange(desc(Year)) %>% 
 arrange(desc(Year),desc(`Qty in M.Ton (Thousands)`))
...
```{r,message=FALSE}
Country_Investigate <- Main %>%  filter(Year==c("2019","2020","2021","2022"))
Countrywise_Summary <- Country_Investigate %>% group_by(`Storage Type`,COUNTRY) %>% summarise(`Qty in M.Ton (Thousands)`= round(sum(`QTY. (MT)`)/1000,2)) %>% arrange(desc(`Qty in M.Ton (Thousands)`))
...
Countrywise_Summary <- Main %>%  filter(Year==c("2019","2020","2021","2022"))
Countrywise_Summary <- Countrywise_Summary %>% group_by(REGION,COUNTRY) %>% summarise(`Qty in M.Ton (Thousands)`= round(sum(`QTY. (MT)`)/1000,2)) %>% arrange(desc(`Qty in M.Ton (Thousands)`))
...
```{r,message=FALSE}
Country_Investigate <- Main %>% filter(Year==c("2019","2020","2021","2022"))
Country_Investigate<- Country_Investigate %>% group_by(COUNTRY) %>% summarise(value=sum(`VALUE ($USD)`)) %>% arrange(desc(value))
kable(Country_Investigate[1,1])
```
...
```{r,message=FALSE,warning=FALSE}
Countrywise_Summary <- Main %>% filter(Year==c("2019","2020","2021","2022"))
Countrywise_Summary <- Countrywise_Summary %>% group_by(Year,COUNTRY) %>% summarise(Value= round(sum(`VALUE ($USD)`)/1000000,2)) %>% arrange(desc(Year),desc(Value))
...
**a. Year wise Summary**
```{r,message=FALSE}
Main_Qtywise_ExporterType <- Main %>% group_by(Year,EXPORTER)%>% summarise(`Qty in M.Ton`=round(sum(`QTY. (MT)`)/1000,2)) %>% arrange(desc(Year))
...
**a. Year wise Summary**
```{r,message=FALSE}
Main_Valuewise_ExporterType <- Main %>% group_by(Year,EXPORTER)%>% summarise(Value=round(sum(`VALUE ($USD)`)/1000000,2)) %>% arrange(desc(Year))
...
**a. Year wise Summary**
```{r,message=FALSE}
Importerwise_Summary <- Main %>% group_by(Year,IMPORTER) %>% summarise(`Qty in M.Tons (Thousands)`= round(sum(`QTY. (MT)`)/1000,2),.groups = 'drop') %>% arrange(desc(Year),desc(`Qty in M.Tons (Thousands)`))
...
**a. Year wise Summary**
```{r,message=FALSE}
Importerwise_Summary <- Main %>% group_by(Year,IMPORTER) %>% summarise(Value= round(sum(`VALUE ($USD)`)/1000000,2),.groups = 'drop') %>% arrange(desc(Year),desc(Value))
